...
# - Queue script: to check newly added, and by the script paused NZBs, and 
#   resume when OK.
...
# Print even more info to the log for debugging(Yes, No).
...
def force_failure(nzb_id):
    ...
    for f in data:  # match files on size for deletion
        ...
        if temp != file_size_low and temp !=par_size_low:
            ...
            if VERBOSE:
                print('[V] Leaving file: ' + str(f.get('Filename')) +
                    ' in the NZB file')
...
def get_nzb_status(nzb):
    ...
    if rar_msg_ids == -1:  # no such NZB file
        ...
        print('[WARNING] The NZB file ' + str(nzb[1]) + ' does not seem to ' + 
            'exist, resuming NZB.')
        ...
        print('Maximum failed articles limit for NZB: ' 
            + str(failed_limit) + '%')
        if MAX_FAILURE > 0:
            print('Maximum failed articles limit for highest level news server: ' 
                + str(MAX_FAILURE) + '%')
        failed_ratio = check_failure_status(rar_msg_ids, failed_limit, nzb[2])
        if VERBOSE:
            print('[V] Total failed ratio: ' + str(round(failed_ratio,1)) + '%')
        if ((failed_ratio < failed_limit and (failed_ratio < MAX_FAILURE or 
                MAX_FAILURE == 0)) or failed_ratio == 0):
            succes = True
            print('Resuming: "' + nzb[1] + '"')
            sys.stdout.flush()
            unpause_nzb(nzb[0])  # unpause based on NZBGet ID
        elif ((failed_ratio >= failed_limit or 
            (failed_ratio >= MAX_FAILURE and MAX_FAILURE > 0)) and 
    ...
    for job in jobs['result']:
        if (job['Status'] == 'DELETED/DUPE' and job['DupeKey'] == nzb[4] and
            ...
            if CHECK_DUPES == 'yes':
                ...
                if VERBOSE:
                    print('[V] DUPE NZB found with lower dupe score, ' + 
    ...
    try:
        ...
        if server_reply in ('411', '420', '423', '430'):
            ...
            print('[ERROR] Socket: ' + str(i) + ' ' + str(host) +
                ', Incorrect news server account settings: ' + str(t))
...
def get_nzb_data(fname):
    ...
    if FULL_CHECK_NO_PARS and par_articles < 1:
        ...
        if temp / each > MAX_ARTICLES:
            ...
            if VERBOSE:
                print('[V] Amount of to be checked articles limited to about ' +
                    str(MAX_ARTICLES) + ' articles.')
        elif temp / each < MIN_ARTICLES:
            each = int(temp / MIN_ARTICLES)
            if each == 0:
                each = 1
            if VERBOSE:
                print('[V] Amount of to be checked articles increased to ' +
                     'about ' + str(MIN_ARTICLES) + ' articles.')
    t = rar_msg_ids[::each]
    rar_msg_ids = t
    # parsing to be used ids, skipping subject parsing
    for i, rar_msg_id in enumerate(rar_msg_ids):
        rar_msg_ids[i][3] = HTMLParser.HTMLParser().unescape(rar_msg_id[3])
...
def get_server_settings(nzb_age):
    ...
    for server_status in servers_status:
        ...
        if retention == '':
            retention = 0
        temp.append([level, group, host, port, username, password,
            encryption, connections, retention, s, active])
    ...
    for server in temp:
        ...
        if server[10] == False:
            skip = True
            if VERBOSE:
                print('[V] Skipping server: ' + server[2] + ', disabled in ' +
                    'NZBGet settings.')
        # Server ID in SERVERS list
        elif (SERVERS[0] != '' and server[9] not in SERVERS and
                server[9] not in FILL_SERVERS):
            skip = True
            if VERBOSE:
                print('[V] Skipping server: ' + server[2] + ', not listed ' +
        ...
        # Server ID in FILL_SERVERS list and nzb older than AGE_LIMIT
            ...
            skip = True
            if VERBOSE:
                print('[V] Skipping Fill server: ' + server[2] +
                    ', NZB age of ' + str(round(nzb_age_days * 24.0,1)) + 
                    ' hours within AgeLimit of ' + str(AGE_LIMIT) + ' hours')
        # Server retention lower than nzb age
        if retention < nzb_age_days and retention != 0:
            skip = True
            if VERBOSE:
                print('[V] Skipping server: ' + server[2] + ', retention of ' +
                    str(retention) + ' days is less than NZB age of ' + 
                    str(round(nzb_age_days,1)) + ' days.')
        # removing all to be skipped servers
        if skip == False:
            servers.append(server)
    if VERBOSE:
        print('[V] All news servers after filtering on Active, Servers, ' + 
            'FillServers + AgeLimit and Retention, ' + 
            ' BEFORE filtering on NZBGet ServerX.Group: ')
        for server in servers:
            print('[V] * ' + str(server[2]) + ':' + str(server[3]) + 
                ', SSL: ' + str(server[6]) + ', connections: ' + 
                str(server[7]))
    ...
    servers = c
    if VERBOSE:
        print('[V] All active news servers AFTER filtering and sorting ' + 
        ...
        for server in servers:
            print('[V] * ' + str(server[2]) + ':' + str(server[3]) + 
                ...
                str(server[7]))
    if servers == []:
        print('[WARNING] No news servers after filtering, marking NZB as' + 
...
def create_sockets(server, articles_to_check):
    '''
        ...
        '''
    if EXTREME:
        print('[E] create_sockets(server=' + str(server) + 
            ',articles_to_check= ' + str(articles_to_check))
    server_no = -1
    conn_err = 0
    server_no += 1
    host = server[2]
    ...
    if end_sock >= articles_to_check:
        ...
        end_sock = num_conn
        if VERBOSE:
            print('[V] Limiting the number of sockets to ' + str(end_sock) + 
    ...
    try:
        ...
        for i in range(start_sock, end_sock):
            ...
            try:
                ...
                print('[WARNING] Socket: ' + str(i) + ' ' + str(e) + 
        ...
        if queue_time != -1:
            ...
            if req_wait > 0:
                if VERBOSE:
                    print('[V] Waiting ' + str(round(req_wait,2)) + ' sec '+
        ...
        if conn_err >= num_conn:
            print('[ERROR] Creation of all sockets for server ' + host +
                ' failed.')
    except:
        print('Exception LINE: ' + 
            str(traceback.tb_lineno(sys.exc_info()[2])) + ': ' +
...
def check_failure_status(rar_msg_ids, failed_limit, nzb_age):
    ...
    articles_to_check = len(rar_msg_ids)
    # message on each 25 %
    message_on = ([1, int(articles_to_check * failed_limit * 0.01),
    ...
    for server in servers:
        ...
        while (send_articles <= articles_to_check - 1 and 
            (failed_ratio < failed_limit or failed_ratio == 0) or
            send_articles <= articles_to_check - 1 and 
            (failed_ratio < MAX_FAILURE or failed_ratio == 0)):
...
#                    # https://bugs.python.org/issue10272
                    ...
                    if socket_loop_count[i] < 5:
                        if EXTREME:
                            print('[E] Socket: ' + str(i) + ' ' + 
                                str(err[0]) + ' ' + str(err[1]))
                            print('[E] Socket: ' + str(i) + ' Failed to ' +
                    ...
                    if socket_loop_count[i] == 5:
                        ...
                        if VERBOSE:
                            print('[V] Socket: ' + str(i) + ' ' + 
                                str(err[0]) + ' ' + str(err[1]))
                            print('[V] Socket: ' + str(i) + ' Still no data ' +
                        ...
                        if failed_wait_count >= 20:
                            print('[WARNING] Skipping current server as ' + 
            ...
            for i in socket_list[m:]:  # loop through ok sockets
                ...
                if reply != None:
                    ...
                    # found ok article on server, store success:
                        ...
                        end_count += 1
                        if end_count >= num_conn:
                            print('All requested article replies received, ' +
                                str(failed_articles) + ' failed.')
        ...
        if len(socket_list) > 0:  # kill still open sockets
            for i in socket_list:
                try:
                    ...
                    continue
            time.sleep(SOCKET_LOOP_INTERVAL)  # waiting and pray the connection will be closed
        print('Failed ratio for server: ' + host + ': ' + 
            str(round(failed_ratio,1)) + '%. Server check completed in ' +
...
def lock_file():
    ...
    if file_exists:
        ...
        if VERBOSE:
            print('[V] time_stamp from completion.lock file= ' 
                + str(time_stamp))  ## added for debug issue reported by blackhawkpr, probably no time stamp in lock file
        fd.close()
        ...
        # Check if the .lock file is not older than 30 minutes
            ...
            print('[ERROR] Script seems to be running for more than 30 ' +
...
def nzbget_paused():
    ...
    if nzbget_paused:
        ...
        while download_rate > 0:  # avoid double use of connections
            ...
            if download_rate == 0:
                if VERBOSE:
                    print('[V] Waiting 5 sec while NZBGet closes the news ' + 
                        'server connections.')
                    sys.stdout.flush()
...
def get_prio_nzb(jobs, paused_jobs):
    ...
    start_time = time.time()
    do_check = False
    max_queued_priority = -1.7976931348623157e+308
    ...
    if VERBOSE and max_queued_priority != -1.7976931348623157e+308:
        print('[V] Maximum priority of DOWNLOADING / QUEUED NZBs = ' + 
    ...
    for job in paused_jobs:
        ...
        if nzb_priority > max_queued_priority:
            do_check = True
            if VERBOSE and max_queued_priority != -1.7976931348623157e+308:
                print('[V] QUEUED / DOWNLOADING NZBs have lower priority ' + 
                    'than by script paused items, starting check')
            break
        else:
            do_check = False
    if do_check == False and max_queued_priority != -1.7976931348623157e+308:
        if VERBOSE:
            print('[V] QUEUED / DOWNLOADING NZBs have higher or equal ' + 
    ...
    if len(jobs['result']) > 0 and 'CnpNZBFileName' in str(jobs):
        if not lock_file():  # check if script is not already running
            ...
            for job in jobs['result']:
                ...
                if ('CnpNZBFileName' in str(job) and 
                    job['Status'] in ('PAUSED')):
...
def queue_call():
    ...
    if (event == 'NZB_ADDED' or event == 'NZB_DOWNLOADED' 
        or event == 'NZB_DELETED' or event == 'NZB_MARKED'):
